...
def segment_image(filtered_array):
    # Manually set a higher threshold value to reduce the tumor size
    threshold_value = threshold_otsu(filtered_array) * 2.0  # Adjust this factor as needed
    tumor_mask = filtered_array > threshold_value

    # Apply morphological operations to further reduce the tumor size
    tumor_mask = binary_erosion(tumor_mask, ball(3))  # Adjust the ball size as needed
...
def display_3d_image(labels):
    # Check if there are any labeled regions to display
    if np.max(labels) == 0:
        messagebox.showerror("Error", "No labeled regions found to display.")
        return
...
result_label = tk.Label(frame, text="", justify=tk.LEFT, anchor="w")
result_label.pack()
